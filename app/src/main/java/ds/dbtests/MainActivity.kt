...
import ds.dbtests.db.greendao.DaoMaster
...
import ds.dbtests.db.objectbox.MyObjectBox
import ds.dbtests.db.objectbox.OrderBox
import ds.dbtests.db.objectbox.UserBox
    ...
    private fun runTests() {
        ...
        create<TestResult> {
            ...
            it.onNext(TestResult("ObjectBox",
                ...
                "ObjectBox full read test", profile { boxStore.runInReadTx { objectBoxReadTestNoSql() } }))
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    private fun objectBoxWriteTestNoSql() {
        val users = boxStore.boxFor(UserBox::class.java)
        val orders = boxStore.boxFor(OrderBox::class.java)
        for (i in 0..ITERATIONS) {
            ...
            for (k in 0..ORDERS) {
                ...
                o.price = 99.95
                o.count = k % 2 + 1
                o.created = Date(System.currentTimeMillis())
    ...
    private fun objectBoxReadTestNoSql() {
        val usersBox = boxStore.boxFor(UserBox::class.java)
        val users = usersBox.all
    ...
    private fun ormaWriteTest() {
        ...
        for (i in 0..ITERATIONS) {
            ...
            for (k in 0..ORDERS) {
                ...
                o.expiration = Date(System.currentTimeMillis() + 1000 * 60)
                o.description = DESCRIPTION
                o.user = SingleAssociation.just(userId, u)
